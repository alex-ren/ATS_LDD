			 ...
			 filldir_t filldir, int short_only, int both)
{
	...
	bh = NULL;
get_new:
        printk (KERN_INFO "myfat: __fat_readdir, get_new round = %d\n", rcount++);
        printk (KERN_INFO "myfat: __fat_readdir, fat_get_entry round = %d\n", readentrycount++);
	if (fat_get_entry(inode, &cpos, &bh, &de) == -1) {
          printk (KERN_INFO "myfat: __fat_readdir, fat_get_entry == -1\n");
          goto end_of_dir;
        }
parse_record:
        printk (KERN_INFO "myfat: __fat_readdir, parse_record round = %d\n", rcount++);
	nr_slots = 0;
	/*
	...
	if (isvfat && !short_only) {
		if (de->name[0] == DELETED_FLAG) {
                    printk (KERN_INFO "myfat: __fat_readdir, jump to record_end 001\n");
			goto record_end;
                }
		if (de->attr != ATTR_EXT && (de->attr & ATTR_VOLUME)) {
                    printk (KERN_INFO "myfat: __fat_readdir, jump to record_end 002\n");
			goto record_end;
                }
		if (de->attr != ATTR_EXT && IS_FREE(de->name)) {
                    printk (KERN_INFO "myfat: __fat_readdir, jump to record_end 003\n");
			goto record_end;
                }
	} else {
		if ((de->attr & ATTR_VOLUME) || IS_FREE(de->name)) {
                    printk (KERN_INFO "myfat: __fat_readdir, jump to record_end 004\n");
			goto record_end;
                }
	}
	...
	if (isvfat && de->attr == ATTR_EXT) {
		...
		if (status < 0) {
			...
			goto out;
                ...
                printk (KERN_INFO "myfat: __fat_readdir, jump to record_end 005\n");
			goto record_end;
                }
		else if (status == PARSE_NOT_LONGNAME)
			goto parse_record;
	...
	if (!last) {
                printk (KERN_INFO "myfat: __fat_readdir, jump to record_end 006\n");
		goto record_end;
        }
...
start_filldir:
        printk (KERN_INFO "myfat: __fat_readdir, start_filldir round = %d\n", rcount++);
...
record_end:
        printk (KERN_INFO "myfat: __fat_readdir, record_end round = %d\n", rcount++);
	furrfu = &lpos;
	filp->f_pos = cpos;
	goto get_new;
end_of_dir:
        printk (KERN_INFO "myfat: __fat_readdir, end_of_dir round = %d\n", rcount++);
	filp->f_pos = cpos;
fill_failed:
        printk (KERN_INFO "myfat: __fat_readdir, fill_failed round = %d\n", rcount++);
	brelse(bh);
	if (unicode)
		__putname(unicode);
out:
        printk (KERN_INFO "myfat: __fat_readdir, out round = %d\n", rcount++);
	unlock_super(sb);
	return ret;
}

// by rzq: fill the dirent as much as possible, may add more than one dir
